...
class Confirm(discord.ui.View):
    ...
    async def confirm(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"> ✅ `{self.instantiator}` ajouté à l'évènement `{event[0]}`")
        # self.stop()
    ...
    async def cancel(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        await self.confirmator.edit(embed=await event_embed(event[0]) )
        await interaction.response.send_message(f"> 🚫 `{self.instantiator}` retiré de `{event[0]}`")
        # self.stop() 
